...
異なる元画像から個別に生成した Panel 168 の単回生成サンプル8点です。短い英語文を使用。横版1536×864は左右各50%、縦版1152×1536は上下各50%です。

確認された偏差：一部は被写体と色面が大きく、紙の余白が不足しています。風景の再現が多く、紙白の負形や枠を越える表現が弱い作例もあります。実際の結果を公開しており、全点のスタイル合格を意味しません。
...
色面を塗る際に紙白を残して主題を作ります。薄塗り、断続的な重ね刷毛、半乾きの擦りと局所的な越境。

## 4つの出力モード

- `top-bottom`：全幅の上下2領域のみ。実写を上、デザインを下に置き、各50%。
- `left-right`：全高の左右2領域のみ。実写を左、デザインを右に置き、各50%。上下構成へ回転しません。
- `design-only`：全画面を Panel 168 のデザイン翻訳にし、写真は見えない参照にします。
...
インストール後に Agent セッションを再起動し、`$xxd-panel-168` を呼び出します。ユーザー単位の Codex には `--global --agent codex --yes` を追加できます。

```text
/xxd-panel-168 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ja-JP
...
完全な実行契約は [SKILL.md](SKILL.md)、実行アダプターは[英語](references/xxd-panel-168-prompt.en.md)／[中国語](references/xxd-panel-168-prompt.zh-CN.md)を参照してください。
