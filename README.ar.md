...
# XXD Panel 168｜حقل مرسوم بطبقات مع حفظ بياض الورق
...
ثمانية نماذج فعلية من Panel 168، وُلّد كل منها مرة واحدة وبشكل مستقل من صورة مصدر مختلفة مع نص إنجليزي قصير. الأفقي 1536×864 بنصفين متساويين يميناً ويساراً، والعمودي 1152×1536 بنصفين متساويين أعلى وأسفل.

الانحرافات الملحوظة: بعض الموضوعات والمساحات اللونية كبيرة وتترك بياضاً غير كافٍ؛ بعض النتائج تحتفظ بتفاصيل المشهد أكثر من المطلوب، مع ضعف الأشكال السلبية البيضاء وتجاوز الحواف. عرض النتائج لا يعني اجتيازها جميعاً لتقييم الأسلوب.
...
يتشكل الموضوع بحفظ بياض الورق أثناء رسم الحقل، مع غسلات رقيقة وطبقات متقطعة وفرك شبه جاف وتجاوز موضعي للإطار.
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 168 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-168`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-168 photo.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-168-prompt.en.md) أو [بالصينية](references/xxd-panel-168-prompt.zh-CN.md).
